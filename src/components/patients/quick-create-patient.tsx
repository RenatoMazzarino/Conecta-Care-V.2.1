...
export function QuickCreatePatient() {
  ...
  const handleClose = async (nextOpen: boolean) => {
    if (!nextOpen) {
      const hasChanges = form.formState.isDirty && !createdId;
      if (hasChanges) {
        const save = window.confirm("Você deseja salvar como rascunho antes de sair?");
        if (save) {
  ...
  const handleCPFBlur = async (cpf: string) => {
    ...
    const { data } = await supabase.from("patients").select("id").eq("cpf", clean).maybeSingle();
    if (data?.id) {
      toast.error("CPF já existe. Abrindo cadastro.");
      setOpen(false);
              ...
              <form className="space-y-6">
                ...
                <section className="bg-white rounded-lg border border-slate-200 shadow-sm p-4 space-y-4">
                  <div className="flex items-center gap-2">
                    <div className="h-6 w-1 bg-slate-400 rounded-full" />
                    <div>
                      <p className="text-sm font-semibold text-slate-800">Documento (Opcional)</p>
                      <p className="text-xs text-slate-500">Envie RG/CPF após criar o registro.</p>
                    </div>
                  </div>
